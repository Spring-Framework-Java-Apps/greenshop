...
<c:forEach var="taxZone" items="${taxZones.content}">
    <c:if test="${taxZone.id == thisTaxZone.id}">
        <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href='<c:url value="/admin/taxZone/${taxZone.id}/edit?page=${taxZones.number}"/>'">
            <td class="dataTableContent"><a href="<c:url value="/admin/taxZone/${taxZone.id}?page=${taxZones.number}"/>"><img src="resources/admin/images/icons/folder.gif" border="0" alt="Folder" title="Folder" /></a>&nbsp;${taxZone.name}</td>
